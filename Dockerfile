...
RUN apt-get update && apt-get install -y --no-install-recommends nginx-light && \
    rm -rf /var/lib/apt/lists/* && rm -f /etc/nginx/sites-enabled/* /etc/nginx/conf.d/*

# Bench + apps (skip redis config so no local redis binary needed at build)
USER frappe
WORKDIR /home/frappe
...
# Runtime: copy + normalize entrypoint to LF, ensure shebang, make exec
USER root
